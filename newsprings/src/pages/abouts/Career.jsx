...
const Career = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true)
      await axios.post(`${import.meta.env.VITE_API_URL}api/career/register`, formData);
  ...
  return (
    <>
      ...
      <div className='about career'>
        ...
        <SectionHighlight
          image={image1}
          title="Let’s impact lives together."
          paragraphs={[
            "At Gateway, we’re all about people because God is all about people. If you want to use your gifts and skills to impact lives and communities, we’d love to connect with you! We have full-time and part-time positions available where you can live out your purpose alongside others who are passionate about Jesus.",
            "Whether you have a background or an interest in administration, creative services, live production, or ministry leadership, there’s a place for you at Gateway."
          ]}
        />
...
<div className="career_testimony_image_container">
  {move > -205 ? (
    <i
      onClick={() => {
        if (move > -205) setMove((prev) => prev - 65.3);
        console.log(move);
      }}
      className="moveright iconactive fa-solid fa-arrow-right"
    ></i>
  ) : null}
  ...
  {move <= -10 ? (
    <i
      onClick={() => {
        if (move <= -10) setMove((prev) => prev + 65.3);
        console.log(move);
      }}
      className="moveleft iconactive fa-solid fa-arrow-left"
    ></i>
  ) : null}

  <div
    className="career_testimony_image"
